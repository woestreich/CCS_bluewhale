...
beuti_daily$month <- month(beuti_daily$date)
beuti_daily$year <- year(beuti_daily$date)
...
for (y in 1988:2021) {
  for (m in 1:12) {
    b <- beuti_daily %>% filter(year == y & month == m) 
    monterey <- sum(b$X37N) 
    cordell <- sum(b$X38N)
...
for (m in 1:12) { 
  ...
  c <- data.frame(monterey_avg, cordell_avg)
  monthly_avgs <- rbind(monthly_avgs, c)}
...
## plot of beuti monthly anomalies 1988 - 2021  
#sums %>%
...
sums %>% filter(year >= 2015 & year <= 2018) %>%
  ...
  ggtitle("BEUTI Monthly Anomaly Comparision") 
